...
import 'viewmodels/cart_provider.dart'; // 🎯 DÜZELTİLDİ: Import yolu standartlaştırıldı
...
void main() async {
  // Flutter yerleşik motorunu garantiye alıyoruz
  WidgetsFlutterBinding.ensureInitialized();

  // Önce veritabanının dolmasını bekliyoruz
  try {
  ...
  // Veriler yüklendikten SONRA uygulamayı ayağa kaldırıyoruz
  runApp(
    MultiProvider(
      providers: [
...
Future<void> _prepareTestData() async {
  ...
  if (existingData.isEmpty) {
    // 1. GRUP: TUVALET KAĞIDI (Barkod: 111)
    ...
    // 2. GRUP: AYÇİÇEK YAĞI (Barkod: 222)
    ...
    // 14. GRUP: DOMATES SALÇASI (Barkod: 1414)
    ...
    // 19. GRUP: SIVI SABUN (Barkod: 1919)
    ...
    // 21. GRUP: KAŞAR PEYNİRİ (Barkod: 2121)
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const LoginScreen(), // 🎯 Giriş ekranıyla başlasın
    );
  }
}