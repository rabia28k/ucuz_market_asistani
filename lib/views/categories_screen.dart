...
class CategoriesScreen extends StatefulWidget {
  // 🎯 DÜZELTİLDİ: Dışarıdan isteğe bağlı seçili kategori parametresi alacak şekilde güncellendi
...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  void initState() {
    ...
    // 🎯 DÜZELTİLDİ: Ana ekrandan bir kategori filtresi geldiyse sadece onu listeler, gelmediyse hepsini gösterir
  ...
  void _filterProducts(String query) {
    setState(() {
      if (query.isEmpty) {
        // Arama temizlendiyse ama ana sayfadan kategori seçildiyse yine kategoriye sadık kalalım
        if (widget.selectedCategory != null) {
        ...
        _filteredProducts = _allProducts.where((product) {
          ...
          // Eğer ana ekrandan kategori kısıtı varsa aramayı o kategorinin içinde yapsın
          if (widget.selectedCategory != null) {
            return product['cat'] == widget.selectedCategory &&
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        // 🎯 Geliştirme: Eğer filtre varsa başlıkta hangi kategoride olduğunu gösterir
            : Text(
            ...
            onPressed: () {
              setState(() {
                if (_isSearching) {
                  ...
                  // Kapatınca ilk filtre durumuna geri dön
                  if (widget.selectedCategory != null) {
                    _filteredProducts = _allProducts.where((p) => p['cat'] == widget.selectedCategory).toList();
                  } else {
                    _filteredProducts = _allProducts;
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Row(
                  children: [
                    Container(
                      margin: const Offset(0, 4) == null ? const EdgeInsets.only(top: 4) : const EdgeInsets.only(top: 4),
